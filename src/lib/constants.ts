...
export interface Field {
  ...
  label: string;
  icon: string;
  apsReqs: Partial<Record<string, number>>;      // Standard NSC 7-pt scale (max 42); UCT = 600-pt scale
  witsIEBReq?: number;                           // Wits IEB scale (Level 8 + bonuses, max ~56)
  notes: string;
}

// witsIEBReq = minimum on Wits IEB scale (Level 8 + LO + bonuses, max ~56)
// apsReqs[wits] = minimum on standard NSC 7-pt scale (for CAPS students, max 42)
export const FIELDS: Field[] = [
  ...
  },
  {
    ...
    apsReqs: { uct: 430, wits: 30, up: 35, su: 38, uj: 30, ukzn: 30, nwu: 28, ufs: 28, ru: 34, uwc: 28, unisa: 20, ul: 26, nmu: 28 },
    witsIEBReq: 38,
